...
def test_maintenance_reminder_not_sent_without_baseline(app):
    ...
    with app.app_context():
        ...
        db.session.add(prefs)
        db.session.add(BatteryHealthRecord(user_id=user.id, vehicle_id=vehicle_id, soh_pct=90, odometer_km=50100))
        db.session.commit()
...
def test_maintenance_reminder_not_sent_when_under_interval(app):
    ...
    with app.app_context():
        ...
        prefs = NotificationPreference(
            user_id=user.id, maintenance_reminders_enabled=True,
            maintenance_interval_km=10000, maintenance_last_service_odometer_km=5010,
        )
        db.session.add(prefs)
